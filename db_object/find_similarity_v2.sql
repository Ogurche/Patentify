...
, IN i_invent_name varchar DEFAULT NULL
...
BEGIN 
	...
	IF inn_matching.valid_patent(i_patent_holder) = FALSE
	THEN 
		o_inn := NULL ;
		o_full_name := NULL;
	...
	SELECT inn
	INTO o_inn 
	FROM inn_matching.patent_request pr
	WHERE 
		...
		FOR x IN SELECT string_to_array(i_patent_holder, ',') LOOP  
			WITH sub AS (
				...
				WHERE 
					p.full_name % x
				LIMIT 5)
			SELECT --TODO протестить все 
				inn_matching.address_matching(inn_agg,mx_simi, x, i_address) AS inn
--				CASE
--					WHEN mx_simi != 1  
...
--				END res 
			...
			END IF; -- если нашли мэтчинг 
		END LOOP; -- если несколько патентователей
	END IF; -- IF NOT FOUND сходств в базе патентов
END
$func$